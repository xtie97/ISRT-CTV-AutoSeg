...
class Segmenter:
    def __init__(
        ...
        if parser.get("sliding_inferrer") is not None:
            ...
            self.sliding_inferrer = SlidingWindowInferer(
                ...
                sw_batch_size=8,  # adjust based on GPU memory
                overlap=0.625,
                mode="gaussian",
    ...
    def train(self):
        ...
        for epoch in range(num_epochs):
            ...
            if (
                config["finetune"]["enabled"]
                and (epoch + 1) % config["num_epochs_per_validation"] == 0
            ):
                run_eval = True

            elif (
                not config["finetune"]["enabled"]
                and (
                    epoch >= 150
                    and (epoch + 1) % config["num_epochs_per_validation"] == 0
                )
            ...
            # avoid potential break in server jobs by saving intermediate checkpoints
