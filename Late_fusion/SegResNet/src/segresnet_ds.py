...
class SegResEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.conv_init_ref = Conv[Conv.CONV, spatial_dims](
        ...
        for i in range(len(blocks_down)):
            ...
            kernel_size = (3, 3, 1) if i == 0 else kernel_size
            padding = (1, 1, 0) if i == 0 else padding
            stride = (2, 2, 1) if i == 0 else stride
        ...
        for i in range(len(blocks_down_ref)):
            level_ref = nn.ModuleDict()
            ...
            blocks_ref = [
                ...
                for _ in range(blocks_down_ref[i])
            ]
            level_ref["blocks"] = nn.Sequential(*blocks_ref)
            level_ref["fusion"] = fusion(
                spatial_dims=spatial_dims,
            ...
            if i < len(blocks_down_ref) - 1:
                level_ref["downsample"] = Conv[Conv.CONV, spatial_dims](
                    in_channels=filters,
                    out_channels=2 * filters,
                    bias=False,
                    kernel_size=kernel_size,
                ...
                level_ref["downsample"] = nn.Identity()

            self.layers_ref.append(level_ref)
            filters *= 2
    ...
    def _forward(self, x: torch.Tensor) -> list[torch.Tensor]:
        ...
        x = self.conv_init(x)
        x_ref = self.conv_init_ref(x_ref)
        outputs = []
        for i, level in enumerate(self.layers):
            x = level["blocks"](x)
            if i < len(self.layers_ref):
                x_ref = self.layers_ref[i]["blocks"](x_ref)
                x_fuse = self.layers_ref[i]["fusion"](torch.cat([x, x_ref], dim=1))
                outputs.append(x_fuse)
                if i == len(self.layers_ref) - 1:
                    x = x_fuse
            else:
                outputs.append(x)
            ...
            if i < len(self.layers_ref):
                x_ref = self.layers_ref[i]["downsample"](x_ref)
