...
class ResidualEncoder(nn.Module):
    def __init__(
        ...
        if not disable_default_stem:
            ...
            self.stem_ref = StackedConvBlocks(
                1,
                conv_op,
                input_channels - 1,
                stem_channels,
        ...
        for s in range(n_stages):
            ...
            stage_ref = (
                StackedResidualBlocks(
                    n_blocks_per_stage[s],  # n_blocks_per_stage[s], 1
                    conv_op,
                    input_channels,
                    features_per_stage[s],
                    kernel_sizes[s],
            ...
            padding = (
                tuple(k // 2 for k in kernel_sizes[s])
                if isinstance(kernel_sizes[s], list)
                else kernel_sizes[s] // 2
            )

            if pool_op is not None:
                stage = nn.Sequential(pool_op(strides[s]), stage)
                stage_ref = (
                    nn.Sequential(pool_op(strides[s]), stage_ref)
            ...
            stages.append(stage)
            stages_ref.append(stage_ref)
            fusions.append(fusion)

            input_channels = features_per_stage[s]

        self.stages = nn.Sequential(*stages)
        self.stages_ref = nn.Sequential(*stages_ref)
    ...
    def forward(self, x, x_ref):
        if self.stem is not None:
            x = self.stem(x)
            x_ref = self.stem_ref(x_ref)
        ret = []
        for s in range(len(self.stages)):
            x = self.stages[s](x)
            if s < len(self.stages) - 1:
                x_ref = self.stages_ref[s](x_ref)
                x_fuse = self.fusions[s](torch.cat([x, x_ref], dim=1))
                ret.append(x_fuse)
                if s == len(self.stages) - 2:
                    x = x_fuse
            else:
                ret.append(x)
