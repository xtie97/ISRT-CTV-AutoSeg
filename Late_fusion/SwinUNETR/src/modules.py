...
class EncoderLayer(nn.Module):
    def __init__(
        ...
        self.self_attn_block_ref = nn.ModuleList(
            [
                SwinTransformerBlock(
                    dim=dim,
                    num_heads=num_heads,
        ...
        self.downsample_ref = downsample
    ...
    def forward(self, x, x_ref):
        ...
        for block in self.self_attn_block:
            x = block(x, attn_mask)

        for block in self.self_attn_block_ref:
            x_ref = block(x_ref, attn_mask)

        # window cross-attention
        for block in self.cross_attn_block:
            x_out = block(x, attn_mask, x_ref)
        ...
        if self.downsample is not None:
            x = self.downsample(x)
            x_ref = self.downsample_ref(x_ref)

        return x, x_ref, x_out
...
class PreResBlock(nn.Module):
    def __init__(
        ...
        self.layerc_ref = UnetrBasicBlock(
            spatial_dims=spatial_dims,
            in_channels=in_chans,
            out_channels=in_chans,
            kernel_size=3,
    ...
    def forward(self, x, x_ref):
        return self.layerc(x), self.layerc_ref(x_ref)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.pos_drop_ref = nn.Dropout(p=drop_rate)
    ...
    def forward(self, x, x_ref, normalize=True):

        x0 = self.patch_embed(x)
        x0 = self.pos_drop(x0)
        x0_ref = self.patch_embed_ref(x_ref)
        x0_ref = self.pos_drop_ref(x0_ref)

        if self.use_v2:
            x0, x0_ref = self.pre_res_block1(x0.contiguous(), x0_ref.contiguous())
        x1, x1_ref, x1_out = self.encoder_layer1(x0.contiguous(), x0_ref.contiguous())
        x1_out = self.proj_out(x1_out, normalize)

        if self.use_v2:
            x1, x1_ref = self.pre_res_block2(x1.contiguous(), x1_ref.contiguous())
        x2, x2_ref, x2_out = self.encoder_layer2(x1.contiguous(), x1_ref.contiguous())
        x2_out = self.proj_out(x2_out, normalize)

        if self.use_v2:
            x2, x2_ref = self.pre_res_block3(x2.contiguous(), x2_ref.contiguous())
        x3, x3_ref, x3_out = self.encoder_layer3(x2.contiguous(), x2_ref.contiguous())
        x3_out = self.proj_out(x3_out, normalize)

        if self.use_v2:
            x3, x3_ref = self.pre_res_block4(x3.contiguous(), x3_ref.contiguous())
        _, _, x4_out = self.encoder_layer4(x3.contiguous(), x3_ref.contiguous())
