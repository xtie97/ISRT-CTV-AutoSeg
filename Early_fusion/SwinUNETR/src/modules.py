...
class EncoderLayer(nn.Module):
    def __init__(
        ...
        self.self_attn_block = SwinTransformerBlock(
            dim=dim,
            num_heads=num_heads,
            window_size=self.window_size,
        ...
        # shift self-attention block
        self.self_attn_block_shift = SwinTransformerBlock(
            dim=dim,
            num_heads=num_heads,
            window_size=self.window_size,
    ...
    def forward(self, x):
        ...
        x = self.self_attn_block(x, attn_mask)
        # shift-window self-attention
        x = self.self_attn_block_shift(x, attn_mask)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.encoder_layer1_down = down_sample_mod(
            dim=int(embed_dim * 1), norm_layer=norm_layer, spatial_dims=spatial_dims
        )
        ...
        self.encoder_layer2_down = down_sample_mod(
            dim=int(embed_dim * 2), norm_layer=norm_layer, spatial_dims=spatial_dims
        )
        ...
        self.encoder_layer3_down = down_sample_mod(
            dim=int(embed_dim * 4), norm_layer=norm_layer, spatial_dims=spatial_dims
        )
    ...
    def forward(self, x, normalize=True):
        ...
        x0 = self.pos_drop(x0)
        x1 = self.encoder_layer1(x0.contiguous())
        x1_out = self.proj_out(x1, normalize)

        x1 = self.encoder_layer1_down(x1.contiguous())
        x2 = self.encoder_layer2(x1.contiguous())
        x2_out = self.proj_out(x2, normalize)

        x2 = self.encoder_layer2_down(x2.contiguous())
        x3 = self.encoder_layer3(x2.contiguous())
        x3_out = self.proj_out(x3, normalize)

        x3 = self.encoder_layer3_down(x3.contiguous())
        x4 = self.encoder_layer4(x3.contiguous())
