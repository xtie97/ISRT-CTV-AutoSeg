...
if use_wandb:
    ...
    # login to wandb using an API key
    wandb.login(key="XXXXXX")  # replace XXXXXX with your wandb API key
...
class DataTransformBuilder:
    ...
    def get_load_transforms(self):
        ...
        # ts.append(DataStatsd(keys=self.image_key, data_shape=True))

        ts.extend(self.get_custom("after_load_transforms"))

        return ts

    # threshold functions for different modalities to crop body region
    def threshold_for_mri(self, x):
        # threshold at 0.0 for T2 MRI
        return x > 0.0

    def threshold_for_ct(self, x):
        # threshold at -200 HU for CT
        return x > -200.0

    def threshold_for_pet(self, x):
        # threshold at 0.2 SUV for PET
        return x > 0.2
            ...
            if self.modality.lower() == "ct":
                ts.append(
                    CropForegroundd(
                        ...
                        select_fn=self.threshold_for_ct,
                        margin=0,
                        allow_missing_keys=True,
                        allow_smaller=True,
                    )
                )
            elif self.modality.lower() == "pet":
                ts.append(
                    CropForegroundd(
                        keys=keys,
                        source_key=self.image_key,
                ...
                raise ValueError("Unsupported modality" + str(self.modality))
    ...
    def get_normalize_transforms(self):
        ...
        for key, normalize_mode in modalities.items():
            ...
            if normalize_mode == "pet":  # SUV input (Save as *1000, uint16)
                ts.append(
                    Lambdad(keys=key, func=lambda x: x / 1000.0)
                )  # rescale to SUV
                intensity_bounds = [0.2, 30]  # clip SUV [0.2, 30] range
                ts.append(
                    ScaleIntensityRanged(
                        keys=key,
    ...
    def get_augment_transforms(self):
        ...
        ts.append(RandGaussianNoised(keys=self.image_key, prob=0.2, mean=0.0, std=0.01))
        ts.append(
            RandGaussianSmoothd(
                keys=self.image_key,
                prob=0.2,
...
class Segmenter:
    def __init__(
        ...
        if config["pretrained_ckpt_name"] is not None:
            start_epoch, start_best_metric = self.checkpoint_load(
                ckpt=config["pretrained_ckpt_name"], model=model
            )
            self.start_epoch = start_epoch + 1
            self.start_best_metric = start_best_metric
    ...
    def parse_input_config(
        ...
        if not torch.cuda.is_available() and config["cuda"]:
            raise ValueError("No cuda is available.! Running on CPU!!!")
            config["cuda"] = False
    ...
    def train(self):
        ...
        use_nnunet_scheduler = config.get("nnunet_scheduler", False)
        if use_nnunet_scheduler:
        ...
        if config["finetune"]["enabled"]:
            ...
            best_metric = self.start_best_metric
            # update the optimizer and lr_scheduler
            if start_epoch > 0:
        ...
        max_train_time = (
            47  # in hours, to prevent timeout in a 48hr job scheduler in the server
        )
        for epoch in range(start_epoch, num_epochs):
            current_time = time.time()
            if (current_time - pre_loop_time > max_train_time * 3600) and use_wandb:
                print("Time limit reached, exiting training")
                ...
                zip_file = os.path.join(
                    pparent_dir, f"ckpt_rng{random_state}_end"
                )  # the zip file to be saved
                shutil.make_archive(zip_file, "zip", parent_dir)
                zip_file = zip_file + ".zip"
                run_wandb.log_model(
                    path=zip_file,
                    name=f"ckpt_rng{random_state}_end",
                    aliases=f"ckpt_rng{random_state}_end",
                )
                os.remove(zip_file)
                break
            ...
            train_acc_mean = float(np.mean(train_acc)) * 100  # in percentage
            if rank == 0:
                print(
                    "Final training  {}/{}".format(epoch, num_epochs - 1),
                    "loss: {:.4f}".format(train_loss),
                    "train acc: {:.2f}%".format(train_acc_mean),  # %
                    "time",
                    train_time,
                )
            ...
            run_eval = False
            # validate every num_epochs_per_validation epochs (defaults to 1, every epoch) after 100 epochs
            if (
                (
                    epoch >= 100
            ...
            if run_eval:
                ...
                val_acc_mean = float(np.mean(val_acc)) * 100  # in percentage
                ...
                if rank == 0:
                    print(
                        ...
                        "val acc: {:.2f}%".format(val_acc_mean),  # %
                        "time",
                        validation_time,
                    )
                    if use_wandb:
                    ...
                    if val_acc_mean > best_metric:
                        print(
                            f"New best metric (%) ({best_metric:.2f} --> {val_acc_mean:.2f}). "
                        )
                    ...
                    if not use_wandb:
                        self.checkpoint_save(
                            ckpt=best_ckpt_path.replace(
                                ".pt", "_{:03d}.pt".format(epoch)
                            ),
                            model=self.model,
                            epoch=epoch,
                            best_metric=best_metric,
            ...
            if epoch == 200 and use_wandb:
                ...
                # zip all the files in the parent_dir
                zip_file = os.path.join(
                    pparent_dir, f"ckpt_rng{random_state}_within200"
                )  # the zip file to be saved
                shutil.make_archive(zip_file, "zip", parent_dir)
                zip_file = zip_file + ".zip"
                run_wandb.log_model(
                    path=zip_file,
                    name=f"ckpt_rng{random_state}_within200",
                    aliases=f"ckpt_rng{random_state}_within200",
                )
                os.remove(zip_file)

            if epoch == 250 and use_wandb:
                parent_dir = os.path.dirname(
                    best_ckpt_path
                )  # the directory where the model.pt is saved
                pparent_dir = os.path.dirname(parent_dir)
                ...
                # zip all the files in the parent_dir
                zip_file = os.path.join(
                    pparent_dir, f"ckpt_rng{random_state}_within250"
                )  # the zip file to be saved
                shutil.make_archive(zip_file, "zip", parent_dir)
                zip_file = zip_file + ".zip"
                run_wandb.log_model(
                    path=zip_file,
                    name=f"ckpt_rng{random_state}_within250",
                    aliases=f"ckpt_rng{random_state}_within250",
                )
                os.remove(zip_file)
    ...
    def validate(self, testing_files=None):
        ...
        val_dice_mean = float(np.mean(val_acc)) * 100  # in percentage
        if self.rank == 0:
            print(
                "Inference complete, loss_avg: {:.4f}".format(val_loss),
                "acc_avg: {:.2f} ".format(val_dice_mean),
    ...
    def infer(self, testing_files=None):
        ...
        # output_path_mask = os.path.join(output_path, "mask")
        # os.makedirs(output_path_mask, exist_ok=True)
        # post_transforms_mask = DataTransformBuilder.get_postprocess_transform(
        #     save_mask=True,
        #     invert=True,
        #     transform=inf_transform,
        #     sigmoid=self.config["sigmoid"],
    ...
    def val_epoch(
        ...
        for idx, batch_data in enumerate(val_loader):
            ...
            if post_transforms:
                ...
                pred = torch.stack(
                    [post_transforms[0](x)["pred"] for x in decollate_batch(batch_data)]
                )
                # pred_mask = torch.stack(
                #     [post_transforms[1](x)["pred"] for x in decollate_batch(batch_data)]
                # )
